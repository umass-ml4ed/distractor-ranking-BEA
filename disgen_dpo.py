        ...
        self.data = [
            {
                ...
                # "label": None if test else get_label(row["gt_distractors"][0], args),
...
class StandardCollator:
    ...
    def __call__(self, batch: List[dict]):
        ...
        if not batch[0].get("label"):
            ...
            # all_inputs = [sample["prompt"] + sample["label"] for sample in batch]
...
def get_dpo_loss_fn(pt_model):
    def dpo_loss(model, batch, args):
        ...
        with torch.no_grad():
            pt_first_log_probs, pt_second_log_probs = get_pairwise_log_probs(pt_model, batch)
...
def finetune(args, fold: int = 0):
    ...
    model, _, tokenizer = get_model(args.base_model, None, None, False)
...
def dpo(args, fold: int = 0):
    ...
    model, pt_model, tokenizer = get_model(args.base_model, None, args.pt_model_name, False)
    train_data, val_data, _ = get_data(fold)
    ...
    train(train_dataloader, val_dataloader, model, get_dpo_loss_fn(pt_model), args)
...
def generate(args, fold: int = 0):
    assert args.model_name
    model, _, tokenizer = get_model(args.base_model, args.model_name, None, True)
...
def ranking_10(args, fold: int = 0):
    assert args.model_name
    model, _, tokenizer = get_model(args.base_model, args.model_name, None, True)
    model.eval()
    test_data = get_data(args, fold)
    ...
    # test_data.to_csv(f"ranking_results/zero_shot_{args.model_name}_average.csv", index=False, encoding='utf-8', quoting=csv.QUOTE_ALL)
    test_data.to_csv(f"ranking_results/ft_{args.model_name}_average.csv", index=False, encoding='utf-8', quoting=csv.QUOTE_ALL)
...
def main():
    ...
    parser.add_argument("--rank_10", action="store_true", help="rank the over-generated distractors")
